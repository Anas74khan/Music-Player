...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Love you Zindagi !!!''')

        self.songProgress = ttk.Scale(top,from_=0)
    ...
    def show_song_details(self):
        ...
        self.songTotalDuration.config(text=str(min)+":"+str(int(sec)))
    ...
    def play_song(self):
        ...
        try:
            ...
            self.songProgress.set(self.song_length)
            self.songProgress.config(to=self.song_length)
            self.setup_thread()
        except (NoSongSelectedError) as ex1:
            messagebox.showerror("Error!",ex1)
    ...
    def add_song_to_favourites(self):
        ...
        try:
            ...
            messagebox.showerror("song cannot be added!!")
    ...
    def show_timer(self,total_sec):
        ...
        while curr_sec<=total_sec:
            if self.isPaused == False:
                ...
                if(curr_sec != 1 and curr_sec != self.songProgress.get()+1):
                    self.song_status(self.songProgress.get())
                    curr_sec = self.songProgress.get()+1
                else:
                    self.songTimePassed.config(text=str(int(min))+":"+str(sec))
                    self.songProgress.set(curr_sec)
                    time.sleep(1)
                    curr_sec+=1
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            mycolor = "#" + red + green + blue
            self.playList.config(fg=mycolor)
            messagebox.showinfo("Favorites loaded!!!","Song loaded successfully!")
        except (NoSongSelectedError) as ex1:
            messagebox.showerror("Error!",ex1)
    ...
    def remove_song_from_favourite(self):
        ...
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song before removing from favorites")
            song_name = self.playList.get(fav_song_index_tuple[0])
            ...
            messagebox.showerror("song cannot be removed!!")
            print(traceback.format_exc())

    def song_status(self,song_time):
        self.my_player.change_song_position(int(song_time))
