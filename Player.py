...
class Player:
    ...
    def add_song(self):
        song_path = filedialog.askopenfilenames(title="Select songs",filetypes=[("mp3 files","*.mp3")])
        if len(song_path) == 0:
            return
        song_name=[]
        for i in song_path:
           name=os.path.basename(i)
           self.my_model.add_song(name,i)
    ...
    def change_song_position(self,song_time):
        mixer.quit()
        mixer.init(frequency=self.audio_tag.info.sample_rate)
        mixer.music.load(self.song_path)
        mixer.music.play(start=(song_time))
